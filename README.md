...
Pawtrol은 당신의 AI 코딩 세션을 지켜보는 보리(Bori)입니다.

이름은 `Paw`와 `Patrol`을 합친 말입니다. 보리는 Codex, Claude Code, Gemini Antigravity, cmux 같은 AI 코딩 에이전트 세션을 순찰하면서 컨텍스트 압박, 토큰 소진 위험, 반복 실패, 개입 타이밍을 귀엽고 직관적인 방식으로 알려줍니다.

RunCat이 CPU 사용량을 메뉴바의 달리는 고양이로 보여주듯, Pawtrol은 AI 코딩 세션의 건강 상태를 화면 위의 작은 반려동물로 보여줍니다. 평소에는 조용히 걷거나 쉬고, 문제가 커질 때는 보리가 짖으면서 상태를 알려줍니다.
