...
Codex 로그인은 Codex CLI의 로그인 흐름을 사용합니다. Pawtrol은 Codex 토큰을 직접 읽지 않고, Codex CLI를 통해 코칭 분석을 요청합니다. API 키가 없어도 Codex auth가 되어 있으면 Codex 기반 코칭을 사용할 수 있고, 연결이 없으면 local heuristic 분석을 사용합니다.
...
`pawtrol`만 실행하면 Bori overlay를 띄우고 실행 중인 Codex, Claude, Antigravity/Gemini 같은 코딩 에이전트를 자동 감지합니다. 아직 API key나 auth 연결이 없으면 터미널에서 연결 방식을 고르는 onboarding 메뉴를 먼저 보여줍니다.
...
이미 다른 터미널에서 실행 중인 에이전트는 프로세스 기반으로 감지합니다. stdout/stderr까지 정밀하게 보려면 아래 `watch -- <command>` 방식을 사용하세요.
...
설정이 끝나면 `pawtrol`과 동일하게 overlay를 바로 띄웁니다. macOS에서는 브라우저를 자동으로 열고, 자동 오픈이 막힌 환경에서는 URL을 출력합니다.

### 에이전트 명령 감시

```bash
pawtrol watch --provider auto -- codex exec "fix failing tests"
pawtrol watch --provider codex -- codex exec "fix failing tests"
...
pawtrol watch --provider codex -- codex run "fix failing tests"
pawtrol watch --provider gemini --model gemini-3-flash-preview -- codex run "fix failing tests"
pawtrol watch --provider openai --model gpt-5.4-mini -- codex run "fix failing tests"
pawtrol watch --provider claude --model claude-sonnet-4-6 -- claude "fix failing tests"
pawtrol watch --provider heuristic -- node scripts/demo-agent.mjs
```
