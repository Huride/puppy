...
export function buildCoachPrompt(signals: SessionSignals): string {
  return [
    ...
    "Use this coaching structure in the content: problem_task, evidence, next_action, and consequence if the session continues unchanged.",
    ...
    "Never execute commands or stop the session automatically; only suggest what the user should do next.",
    ...
    JSON.stringify(
      {
        ...
        evidence: ["근거 1", "근거 2"],
        next_action: "지금 할 일. 예: 전체 테스트 대신 auth.spec.ts 단일 테스트와 token.ts 변경만 확인",
